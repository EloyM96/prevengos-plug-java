-- -----------------------------------------------------------------------------
-- Esquema PostgreSQL v1.0.0
-- Objetivo: almacenar eventos canónicos y entidades normalizadas sincronizadas
-- desde Prevengos hacia el hub PRL.
-- -----------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_event_envelopes_type_occurred
    ON event_envelopes (event_type, occurred_at DESC);
CREATE INDEX IF NOT EXISTS idx_event_envelopes_correlation
...
CREATE INDEX IF NOT EXISTS idx_cuestionarios_estado ON cuestionarios (estado);
...
CREATE INDEX IF NOT EXISTS idx_cuestionario_respuestas_pregunta
...
-- Vista materializada para consultas frecuentes de resumen de pacientes
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_pacientes_citas AS
SELECT
    p.paciente_id,
    p.nombre,
    p.apellidos,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_pacientes_citas ON mv_pacientes_citas (paciente_id);
